...
var flatten = function(options) {
    var moduleInfo = { licenses: license.UNKNOWN },
        json = options.deps,
        data = options.data,
    ...
    if (json.path && fs.existsSync(json.path)) {
        ...
        files = licenseFiles(dirFiles, moduleInfo.licenses);
    ...
    files.forEach(function(filename, index) {
        ...
        var content;
        if (license.priority(moduleInfo.licenses) !== license.priority.KNOWN) {
            //Only re-check the license if we didn't get it from elsewhere
            content = fs.readFileSync(licenseFile, { encoding: 'utf8' });
            moduleInfo.licenses = license.best(moduleInfo.licenses, license(content));
        }
    ...
    read(options.start, opts, function(err, json) {
        ...
        Object.keys(data).sort().forEach(function(item) {
            if (data[item].private) {
                data[item].licenses = colorizeString(license.UNLICENSED);
            }
            /*istanbul ignore next*/
            if (!data[item].licenses) {
                data[item].licenses = colorizeString(license.UNKNOWN);
            }
            if (options.unknown) {
                /*istanbul ignore else*/
                if (data[item].licenses && data[item].licenses !== license.UNKNOWN) {
            ...
            if (data[item]) {
                if (options.onlyunknown) {
                    if (data[item].licenses.indexOf('*') > -1 ||
                        data[item].licenses.indexOf(license.UNKNOWN) > -1) {
            ...
            Object.keys(sorted).forEach(function(item) {
                ...
                if(!licenses) {
                    ...
                    licenses.forEach(function(license) {
                        /*istanbul ignore if - just for protection*/
                        if (license.indexOf(license.UNKNOWN) >= 0) { // necessary due to colorization
                            filtered[item] = sorted[item];
                        } else {
