...
function licensePriority(license) {
    if (!license) {
        return PRIORITY.NONE;
    }
    if (Array.isArray(license)) {
        return license.reduce(function(acc, lic) {
            return Math.max(acc, licensePriority(lic));
        }, PRIORITY.NONE);
    }
    if (typeof license !== 'string') {
        console.error("Unexpected license (type: " + (typeof license) + "): " + license);
        return PRIORITY.ERROR;
    }
    if (license.indexOf(UNKNOWN) > -1) {
    ...
    if (license.endsWith('*')) {
        return PRIORITY.GUESS;
    }
    return PRIORITY.KNOWN;
}

function bestLicense(/*license-1-or-licenses/*, license-2-or-none, ...*/) {
    var licenses;
    if (arguments.length === 1 && Array.isArray(arguments[0])) {
        licenses = arguments[0];
    } else {
        licenses = Array.prototype.slice.call(arguments);
    }
    return licenses.reduce(function(best, license) {
        if (licensePriority(license) > licensePriority(best)) {
            return license;
        } else {
            return best;
        }
...
Object.assign(module.exports, {
    ...
    priority: Object.assign(licensePriority, PRIORITY),
    best: bestLicense,
});
